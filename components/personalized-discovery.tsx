...
export function PersonalizedDiscovery() {
  const { hasMembership, isAuthenticated, session } = useMembershipAccess()
  const router = useRouter()
  const authModal = useAuthModal()

  const subtitle = useMemo(() => {
    if (isAuthenticated && session?.user?.name) {
      return `Xin chào ${session.user.name}, đây là những gợi ý được cập nhật theo lịch rảnh & wishlist của bạn.`
    }
    return "Thuật toán gợi ý dùng dữ liệu đặt phòng, lượt lưu và các theme yêu thích để đề xuất nơi ở, trải nghiệm và dịch vụ đính kèm phù hợp nhất."
  ...
  const handleCta = () => {
    ...
    if (!hasMembership) {
      router.push("/membership?ref=personalized-discovery")
      return
    }
    router.push("/experiences/members?ref=personalized-discovery")
  }
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div className="space-y-6">
            ...
            <h2 className="font-serif text-4xl font-bold text-slate-900 md:text-5xl">Khám phá trải nghiệm độc đáo</h2>
            <Badge variant={hasMembership ? "secondary" : "outline"} className="w-fit uppercase tracking-wide text-[11px]">
              {hasMembership ? "Đã mở khóa membership" : "Yêu cầu membership"}
            </Badge>
            ...
            <Button
              variant={hasMembership ? "default" : isAuthenticated ? "outline" : "default"}
              className="rounded-xl px-6 py-6 text-base font-semibold shadow-lg shadow-primary/20"
              onClick={handleCta}
            >
              {!isAuthenticated ? "Đăng nhập để xem gợi ý" : hasMembership ? "Xem gợi ý dành riêng" : "Nâng cấp membership để xem gợi ý"}
            </Button>
            {!hasMembership && (
              <p className="text-sm text-slate-500">
                Gợi ý cá nhân hóa nằm trong gói membership. Đăng ký để đồng bộ wishlist và lịch rảnh của bạn.
              </p>
            )}
          </div>
