...
import { Star, Users, CreditCard, ChevronDown, Calendar } from "lucide-react"
...
export function BookingWidget({ listingId, price, rating, reviews, instantBookable = true }: BookingWidgetProps) {
  ...
  const handleServicesChange = (totalPrice: number, services: any[]) => {
      ...
      <CardContent className="space-y-4">
        ...
        {!session?.user && (
          <div className="rounded-lg bg-muted/40 border border-dashed border-border p-3 text-xs text-muted-foreground">
            <span>
              Đăng nhập để tự động điền thông tin và nhận ưu đãi dành cho thành viên.
            </span>
            <Button
              type="button"
              variant="link"
              className="h-auto px-0 ml-2 text-primary"
        ...
        {nights > 0 && (
          <div className="space-y-3 pt-4 border-t border-border">
            ...
            {servicesTotal > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">
                  Dịch vụ bổ sung ({selectedServices.length})
                </span>
